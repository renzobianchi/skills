...
One short pass BEFORE the fan out whose output is a shared **grounding memo**: which patterns exist in production for this problem, by name. Candidates receive it as vocabulary, with the instruction to diverge from it, not to copy the first reference.

Source ladder (use the highest available; the memo states which one ran):
...
## Craft layer (installed skills)

Before the fan out, detect which craft skills exist in the skills directory (`ls`). The memo states which were found; with the ones that exist:
...
One readonly judge (a model different from the parent's: `opus`/`sonnet`/`haiku`) that sees the rubric and **the rendered artifacts** (screenshots per medium; see the reference), never the rationales alone. It scores per criterion, recommends a base, and names the best graft candidate per candidate.
...
One more pass over each loser: one or two grafts per candidate, not most of it. Graft by hand keeping a single mental model; a Frankenstein of directions fails the whole rubric. Record in the synthesis note: base, grafts with source, rejections with reason (the most valuable part of the record), dropouts.
...
- **Base + grafts, always.** The midpoint between two good layouts is usually a bad one: pick one whole base and port pieces into it, however strong the pull to blend halves.
- **Screenshot before scoring, always.** Judging without rendering is judging the rationale, and rationales always sound convincing.
- **Paper and Figma have real shared state** (a single file/page): the output separation from Phase A is the only thing keeping candidates from clobbering each other. Quotas and write gotchas live in each medium's reference.
- **Real content goes in the frame**: a layout picked with placeholder content is invalidated when the real content arrives.
