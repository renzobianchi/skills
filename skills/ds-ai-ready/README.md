...
adapters/
  claude/skills/        ds-rules (auto), ds-migrate (/ds-migrate <phase>), ds-patterns, ds-help (where am I); ds-rules and references/ generated from core/
...
- **Claude Code plugin.** `/plugin marketplace add renzobianchi/skills` then `/plugin install ds-ai-ready@renzobianchi-skills`; from a local checkout, `claude --plugin-dir /path/to/ds-ai-ready`. Skills are namespaced: `/ds-ai-ready:ds-migrate foundations`. Lost? `/ds-ai-ready:ds-help` reads the repo and prints the phase, what blocks it and the next command.
- **With the skills pack.** `git clone https://github.com/renzobianchi/skills && sh skills/install.sh` copies `ds-rules`, `ds-migrate`, `ds-patterns` and `ds-help` flat into every agent skill directory it finds. Invoke as `/ds-migrate foundations`.
...
At any point, `/ds-ai-ready:ds-help` says which of these steps you are on and what to run next.
...
## Parity is a stamp, not a word

A `parity` manifest carries `verified { level, at, kitVersion }`, written by `node scripts/ds-manifest.mjs verify <key> <kitVersion>` after the audit. `check` refuses one without it, or with an axis that differs from the kit and no note; `audit code` compares the axes with the source's cva variants; `check --kit-version <current>` names what the kit has moved past. Cleanup re-audits every entry before deleting legacy, steward repeats it quarterly.
