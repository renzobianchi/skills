...
**Done when:** the build procedure's own criterion holds: every recorded measurement matches in both modes in the browser, and the kit version id you audited against is written down for step 5 (Figma: the file version from the bridge; Paper: the project commit).
...
- Stamp the mirror: `node scripts/ds-manifest.mjs verify <key> <kitVersion>` with the version id from step 3. It refuses while an axis drifts unnoted. Then `node scripts/ds-manifest.mjs audit code`: the manifest's axes must match the `cva` variants in the source. A manifest that says `parity` and fails either is the defect the QA pass finds later.
...
**Done when:** `node scripts/ds-manifest.mjs check --kit-version <kitVersion>` prints `ok` (verified at `parityLevel`, axes mirrored or noted, usage doc present, zero placeholders), `audit code` prints `ok`, Owner notes is filled or says "none yet", and the diff touches no manifest file other than this component's and the legacy exports it replaces.
